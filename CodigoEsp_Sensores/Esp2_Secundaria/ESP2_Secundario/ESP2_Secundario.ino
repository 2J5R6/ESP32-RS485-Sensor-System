...
// Variables de sensores (sin MPU6050)
struct SensorData {
  float potenciometro;
  float ldr;
  float encoder;
...
void setup() {
  ...
  Serial.println("Sistema RS485/RS232 con sensores múltiples");
  ...
  Serial.println("✓ ESP2 listo para operación (MPU6050 deshabilitado)");
  Serial.println("Frecuencia de actualización: 20Hz (50ms)");
}
...
void processPC_Commands() {
  if (Serial.available()) {
    ...
    if (!error && cmd["target"] == "ESP2") {
      ...
      Serial.println("{\"ack\":\"ESP2\",\"cmd\":\"" + command + "\"}");
    }
  }
}
