 ...
 *   • Potenciómetro: pin 8
 *   • LDR: pin 18  
 *   • Encoder/Velocidad: pin 17
 * - MPU6050 I2C:
 *   • SDA: pin 43, SCL: pinvoid parseRS485Message() {
  ...
  if (!error) {
    ...
    if (doc["type"] == "command") {
      ...
      if (!cmdError && cmd["target"] == "ESP1") {
        Serial.println("DEBUG - Ejecutando comando para ESP1 desde RS485");
        processLEDCommands(cmd, commandData);
      }
    }
    // Verificar si son datos de sensores
    else if (doc["src"] == "ESP2") {
      Serial.println("DEBUG - Datos de ESP2 recibidos correctamente");
      // Actualizar datos remotos de ESP2
 ...
 * - RS485:
 *   • RO (Receptor Output): pin 7
 *   • RE (Receptor Enable): pin 6  
 *   • DE (Driver Enable): pin 5
 *   • DI (Driver Input): pin 4
 * - LEDs de Control:
 *   • LED Verde: pin 12
 *   • LED Rojo: pin 11
 *   • LED Amarillo: pin 10
 ...
 * - RS485: Half-dúplex alternado con ESP2
 * - Comandos LED desde PC interpretados en tiempo real
 */

#include <Wire.h>
// #include <MPU6050.h>  // Temporalmente deshabilitado
#include <ArduinoJson.h>
#include <HardwareSerial.h>

// ============ DEFINICIÓN DE PINES ============
// Sensores Analógicos
#define PIN_POTENCIOMETRO   8
#define PIN_LDR            18
#define PIN_ENCODER        17
...
// RS485 Half-Duplex
#define RS485_RO           7   // Receptor Output
#define RS485_RE           6   // Receptor Enable (activo bajo)
#define RS485_DE           5   // Driver Enable (activo alto)
#define RS485_DI           4   // Driver Input
...
// MPU6050 mpu;  // Temporalmente deshabilitado
...
bool rs485_tx_mode = true;  // Alternar entre TX y RX
...
void setup() {
  ...
  Serial.println("=== ESP32-S3 ESP1 Iniciando ===");
  ...
  setRS485ReceiveMode();  // Iniciar en modo recepción
  ...
  // Wire.begin(SDA_PIN, SCL_PIN);  // MPU6050 deshabilitado temporalmente
  ...
  Serial.println("* ESP1 listo para operacion");
...
void loop() {
  ...
  // Alternar entre envío y recepción cada 25ms (40Hz total, 20Hz cada dirección)
  if (rs485_tx_mode && (currentTime - lastRS485Send >= UPDATE_INTERVAL)) {
    sendDataViaRS485();
    lastRS485Send = currentTime;
    rs485_tx_mode = false;  // Cambiar a modo recepción
    setRS485ReceiveMode();
  } 
  else if (!rs485_tx_mode && (currentTime - lastRS485Receive >= UPDATE_INTERVAL)) {
    receiveDataViaRS485();
...
void readAllSensors() {
  ...
  // MPU6050 temporalmente deshabilitado
  // int16_t ax, ay, az, gx, gy, gz;
  // mpu.getMotion6(&ax, &ay, &az, &gx, &gy, &gz);
  // localSensors.accel_x = ax / 16384.0;
  // localSensors.accel_y = ay / 16384.0;
  // localSensors.accel_z = az / 16384.0;
  // localSensors.gyro_x = gx / 131.0;
...
void parseRS485Message() {
  ...
  if (!error && doc["src"] == "ESP2") {
    ...
    Serial.print("DEBUG - Error parseando RS485 o fuente incorrecta: ");
    Serial.println(error.c_str());
  }
}
...
void processPC_Commands() {
  if (Serial.available()) {
    ...
    if (!error) {
      // Si el comando es para ESP1 (esta tarjeta)
      if (cmd["target"] == "ESP1") {
        Serial.println("DEBUG - Comando para ESP1 procesado");
        processLEDCommands(cmd, command);
      }
      // Si el comando es para ESP2, reenviarlo por RS485
      else if (cmd["target"] == "ESP2") {
        Serial.println("DEBUG - Reenviando comando para ESP2 via RS485");
...
void processLEDCommands(StaticJsonDocument<128>& cmd, String& originalCommand) {
  ...
  if (commandProcessed) {
    Serial.println("{\"ack\":\"ESP1\",\"cmd\":\"" + originalCommand + "\"}");
  }
}
